...
optix::float3 triangle_math::uv2xyz(int triangleId, optix::float2 &uv, std::vector<Vertex> &vertices) {
...
glm::vec3 triangle_math::calculateCentre(float triangleId, std::vector<Vertex> &vertices) {
	glm::vec3 centre = (vertices[triangleId * 3].pos + vertices[triangleId * 3 + 1].pos + vertices[triangleId * 3 + 2].pos);
	return glm::vec3(centre.x / 3, centre.y / 3, centre.z / 3);
}

glm::vec3 triangle_math::avgNormal(float triangleId, std::vector<Vertex> &vertices) {
...
float triangle_math::calculateSurface(int triangleId, std::vector<Vertex> &vertices) {
...
std::vector<std::vector<glm::vec3>> triangle_math::divideInFourTriangles(float triangleId, std::vector<Vertex> &vertices) {
	glm::vec3 a = vertices[triangleId * 3].pos;
	glm::vec3 b = vertices[triangleId * 3 + 1].pos;
	glm::vec3 c = vertices[triangleId * 3 + 2].pos;
...
bool triangle_math::isFacingBack(glm::vec3 origin, float destPatch, std::vector<Vertex> &vertices) {
	// check if facing back of triangle:
	glm::vec3 center_dest = triangle_math::calculateCentre(destPatch, vertices);
	glm::vec3 normal_dest = triangle_math::avgNormal(destPatch, vertices);
