...
public:
	...
	optix::float3 un_project(double winx, double winy, double winz, glm::mat4x4 & modelview, glm::mat4x4 & projection, glm::vec4 & viewport)
	{
		// Calculation for inverting a matrix, compute projection x modelview
		// and store in A
		glm::mat4x4 A = projection * modelview;
		// Now compute the inverse of matrix A
		glm::mat4x4 m = glm::inverse(A); 
		// Transformation of normalized coordinates between -1 and 1
		double in0 = (winx - (double)viewport[0]) / (double)viewport[2] * 2.0 - 1.0;
		double in1 = (winy - (double)viewport[1]) / (double)viewport[3] * 2.0 - 1.0;
		double in2 = 2.0*winz - 1.0;
		double in3 = 1.0;
		glm::vec4 in(in0, in1, in2, in3);
		// Objects coordinates
		glm::vec4 out = m * in;
		double out3 = 1.0 / out[3];
		float objc0 = out[0] * out3;
		float objc1 = out[1] * out3;
		float objc2 = out[2] * out3;
		optix::float3 objectCoordinate = { objc0, objc1, objc2 };
		return objectCoordinate;
	}


	glm::mat4x4 ortho(double left, double right, double bottom, double top, double zNear, double zFar){
		float m0 = 2.0f / (right - left);
		float m4 = 0.0f;
		float m8 = 0.0f;
		float m12 = 0.0f;

		float m1 = 0.0f;
		float m5 = 2.0f / (top - bottom);
		float m9 = 0.0f;
		float m13 = 0.0f;

		float m2 = 0.0f;
		float m6 = 0.0f;
		float m10 = -2.0f / (zFar - zNear);
		float m14 = 0.0f;
		...
		glm::mat4x4 res(m0, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14, m15);
		return res;
	}
